...
export default function DailyFactFormModal({
  ...
  const handleSubmit = async () => {
    if (!monthlyPlan?.id) {
      setErrorText("Oylik plan topilmadi");
      return;
    }
    ...
    try {
      ...
      await api.post("/daily-facts", {
        monthlyPlanId: monthlyPlan.id,
        userId: monthlyPlan.user.id,
        date: today,
      ...
      console.error("Fakt saqlashda xatolik:", error);
      setErrorText(
        error?.response?.data?.message || "Kunlik fakt saqlashda xatolik"
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Modal open={open} onClose={onClose} title="Kunlik fakt yozish">
      <div className="space-y-5">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <TextInput
            label="Fakt qo‘ng‘iroq"
            type="number"
